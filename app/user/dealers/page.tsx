...
export default function SearchDealers() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.dealers && res.dealers.length > 0) {
        setSelectedDealer(res.dealers[0]);
      } else {
  ...
  // Set default selected dealer if not set
  if (!selectedDealer && dealers && dealers.length > 0) {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-foreground">Certified Scrap Dealers</h2>
          <p className="text-xs text-muted-text mt-1 uppercase tracking-wider font-semibold">India's Smartest Certified Recycling Network</p>
        </div>
      ...
      <div className="glassmorphism p-4.5 bg-card border border-card-border shadow-sm">
        <MapPreview
          ...
          onSelectLocation={setSelectedDealer}
          userLocationInput={activeSearch}
          typeLabel="Dealers"
        />
      </div>

      {/* Complete Dealers list */}
      <div className="space-y-4">
        <h3 className="text-sm font-black text-foreground uppercase tracking-wider">All Certified E-Waste Dealers Near {activeSearch}</h3>
        
        {dealers.length === 0 ? (
          <div className="glassmorphism p-12 text-center text-muted-text">
            No e-waste dealers found near "{activeSearch}". Try searching for major cities like "Delhi", "Mumbai", or "Bangalore".
          </div>
        ) : (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[11px] text-muted-text font-semibold mt-1">Owner: {dealer.ownerName}</p>
                    </div>
                  ...
                  <div className="mt-4 text-xs text-foreground space-y-2 border-t border-card-border pt-4">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <div className="font-bold text-foreground">{dealer.distance} away</div>
                        <div className="text-muted-text text-[11px] mt-0.5">{dealer.address}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-6 border-t border-card-border pt-4">
                    ...
                    <a
                      href={`https://wa.me/${dealer.phone.replace(/[^0-9]/g, "")}?text=Hi,%20I%20am%20interested%20in%20selling%20my%20e-waste%20with%20ScrapSense.`}
                      target="_blank"
